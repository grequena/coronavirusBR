  ...
  ggplot2::annotate(geom="text",
                    ...
                    label = paste("Oceania:",
                                  format(total$con[total$continent=="Oceania"],
                                         big.mark = ".", decimal.mark = ",")))+  # AFRICA
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$con)[1:4]), 
                    xend = lubridate::ymd(max(df_cont$date))                -12, 
                    yend = sum(rev(total$con)[1:4]),
                    size = .2, linetype = "dashed",
                    curvature = 0) +
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -12,
                    y = sum(rev(total$con)[1:4]), size=3,hjust = 1,
                    label = paste("Africa:",
...
fig <- df_cont %>% 
  ...
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -12,
                    y = sum(rev(total$rec)[1:4]), size=3,hjust = 1,
                    label = paste("Africa:",
...
fig <- df_cont %>% 
  ...
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -12,
                    y = sum(rev(total$dea)[1:4]), size=3,hjust = 1,
                    label = paste("Africa:",
