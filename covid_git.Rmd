...
### **<span style="color: #1478BE;">CONFIRMADOS</span> por continente**

<<<<<<< HEAD
```{r}
=======
```{r fig.width=8, fig.height=3}
>>>>>>> 8b66d32497d2c997dcbc96fcb75fcbcc33ed3923
...
fig <- df_cont %>% 
  ...
  # OCEANIA seta curva + texto
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$con)[1:5]), 
                    xend = lubridate::ymd(max(df_cont$date))                -2, 
                    yend = sum(rev(total$con)[1:5]), 
                    curvature = 0, 
                    #arrow=
                    size = .15, linetype = "dashed") +
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -2,
                    y = sum(rev(total$con)[1:5]),
                    size=4, hjust = 1,
                    label = paste("Oceania:",
                                  format(total$con[total$continent=="Oceania"],
                                         big.mark = ".", decimal.mark = ",")))+ 
  # AFRICA
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$con)[1:4]), 
                    xend = lubridate::ymd(max(df_cont$date))                -19, 
                    yend = sum(rev(total$con)[1:4]),
                    size = .15, linetype = "dashed",
                    curvature = 0) +
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -19,
                    y = sum(rev(total$con)[1:4]), size=4,hjust = 1,
                    label = paste("Africa:",
                                  format(total$con[total$continent=="Africa"],
                                         big.mark = ".", decimal.mark = ",")))+
  # AMERICAS
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$con)[1:3]), 
                    xend = lubridate::ymd(max(df_cont$date))                -35, 
                    yend = sum(rev(total$con)[1:3]), 
                    size = .15, linetype = "dashed", 
                    curvature = 0) +
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -35,
                    y = sum(rev(total$con)[1:3]), 
                    size=4,hjust = 1,
                    label = paste("Americas:",
                                  format(total$con[total$continent=="Americas"],
                                         big.mark = ".", decimal.mark = ",")))+
  # EUROPE
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$con)[1:2]), 
                    xend = lubridate::ymd(max(df_cont$date))                -7, 
                    yend = sum(rev(total$con)[1:2]),
                    size = .15, linetype = "dashed",
                    curvature = 0)+
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -7,
                    y = sum(rev(total$con)[1:2]), 
                    size=4,hjust = 1,
                    label = paste("Europe:",
                                  format(total$con[total$continent=="Europe"],
                                         big.mark = ".", decimal.mark = ",")))+
  # ASIA
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = rev(total$con)[1], 
                    xend = lubridate::ymd(max(df_cont$date))                -20, 
                    yend = rev(total$con)[1],
                    size = .15, linetype = "dashed",
                    curvature = 0)+
  ggplot2::annotate(geom="text", 
                    size=4,hjust = 1,
                    x = lubridate::ymd(max(df_cont$date))                   -20,
                    y = rev(total$con)[1],
                    label = paste("Asia:",
                                  format(total$con[total$continent=="Asia"],
                                         big.mark = ".", decimal.mark = ",")))
fig
```

### **<span style="color:#0A780AE6">RECUPERADOS</span> por continente**
...
fig <- df_cont %>% 
  ...
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$rec)[1:5]), 
                    xend = lubridate::ymd(max(df_cont$date))                -2, 
                    yend = sum(rev(total$rec)[1:5]), 
                    curvature = 0, 
                    #arrow=
                    size = .15, linetype = "dashed") +
  ggplot2::annotate(geom="text",
                    ...
                    size=4, hjust = 1,
                    label = paste("Oceania:",
                                  format(total$rec[total$continent=="Oceania"],
                                         big.mark = ".", decimal.mark = ",")))+
  # AFRICA
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$rec)[1:4]), 
                    xend = lubridate::ymd(max(df_cont$date))                -19, 
                    yend = sum(rev(total$rec)[1:4]),
                    size = .15, linetype = "dashed",
                    curvature = 0) +
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -19,
                    y = sum(rev(total$rec)[1:4]), size=4,hjust = 1,
                    label = paste("Africa:",
                                  format(total$rec[total$continent=="Africa"],
                                         big.mark = ".", decimal.mark = ",")))+
  # AMERICAS
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$rec)[1:3]), 
                    xend = lubridate::ymd(max(df_cont$date))                -35, 
                    yend = sum(rev(total$rec)[1:3]), 
                    size = .15, linetype = "dashed", 
                    curvature = 0) +
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -35,
                    ...
                    size=4,hjust = 1,
                    label = paste("Americas:",
                                  format(total$rec[total$continent=="Americas"],
                                         big.mark = ".", decimal.mark = ",")))+
  # EUROPE
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$rec)[1:2]), 
                    xend = lubridate::ymd(max(df_cont$date))                -3, 
                    yend = sum(rev(total$rec)[1:2]),
                    size = .15, linetype = "dashed",
                    curvature = 0)+
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -3,
                    y = sum(rev(total$rec)[1:2]), 
                    size=4,hjust = 1,
                    label = paste("Europe:",
                                  format(total$rec[total$continent=="Europe"],
                                         big.mark = ".", decimal.mark = ",")))+
  # ASIA
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = rev(total$rec)[1], 
                    xend = lubridate::ymd(max(df_cont$date))                -13, 
                    yend = rev(total$rec)[1],
                    size = .15, linetype = "dashed",
                    curvature = 0)+
  ggplot2::annotate(geom="text", 
                    size=4,hjust = 1,
                    x = lubridate::ymd(max(df_cont$date))                   -13,
                    y = rev(total$rec)[1],
                    label = paste("Asia:",
                                  format(total$rec[total$continent=="Asia"],
                                         big.mark = ".", decimal.mark = ",")))
fig
```

### **<span style="color:#DC2828E6">MORTES</span> por continente**
...
fig <- df_cont %>% 
  ...
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$dea)[1:5]), 
                    xend = lubridate::ymd(max(df_cont$date))                -2, 
                    yend = sum(rev(total$dea)[1:5]), 
                    curvature = 0, 
                    #arrow=
                    size = .15, linetype = "dashed") +
  ggplot2::annotate(geom="text",
                    ...
                    size=4, hjust = 1,
                    label = paste("Oceania:",
                                  format(total$dea[total$continent=="Oceania"],
                                         big.mark = ".", decimal.mark = ",")))+
  # AFRICA
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$dea)[1:4]), 
                    xend = lubridate::ymd(max(df_cont$date))                -19, 
                    yend = sum(rev(total$dea)[1:4]),
                    size = .15, linetype = "dashed",
                    curvature = 0) +
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -19,
                    y = sum(rev(total$dea)[1:4]), size=4,hjust = 1,
                    label = paste("Africa:",
                                  format(total$dea[total$continent=="Africa"],
                                         big.mark = ".", decimal.mark = ",")))+
  # AMERICAS
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$dea)[1:3]), 
                    xend = lubridate::ymd(max(df_cont$date))                -35, 
                    yend = sum(rev(total$dea)[1:3]), 
                    size = .15, linetype = "dashed", 
                    curvature = 0) +
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -35,
                    ...
                    size=4,hjust = 1,
                    label = paste("Americas:",
                                  format(total$dea[total$continent=="Americas"],
                                         big.mark = ".", decimal.mark = ",")))+
  # EUROPE
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
                    y = sum(rev(total$dea)[1:2]), 
                    xend = lubridate::ymd(max(df_cont$date))                -3, 
                    yend = sum(rev(total$dea)[1:2]),
                    size = .15, linetype = "dashed",
                    curvature = 0)+
  ggplot2::annotate(geom="text",
                    x = lubridate::ymd(max(df_cont$date))                   -3,
                    ...
                    size=4,hjust = 1,
                    label = paste("Europe:",
                                  format(total$dea[total$continent=="Europe"],
                                         big.mark = ".", decimal.mark = ",")))+
  # ASIA
  ggplot2::annotate(geom = "curve", 
                    x    = lubridate::ymd(max(df_cont$date)),       
  ...
  ggplot2::annotate(geom="text", 
                    ...
                    y = rev(total$dea)[1],
                    label = paste("Asia:",
                                  format(total$dea[total$continent=="Asia"],
                                         big.mark = ".", decimal.mark = ",")))
fig
```
...
### **Evolução dos CONFIRMADOS desde `r as.character.POSIXt(min(coronavirus$date), format = "%d/%m/%Y")`**
...
### **Evolução desde o dia do 500º CONFIRMADO **
```{r}
df_pais_500 <- df_pais %>%
  dplyr::group_by(Country.Region) %>%
...
fig2 <- plotly::layout(fig2,
                       title = "",
                       yaxis = list(title = 
                                      "Frequência acumulada de CONFIRMADOS"),
                       xaxis = list(title = "Dia após o 500º caso CONFIRMADO"),
                       legend = list(x=0.1, y=0.9),
                       hovermode = "compare")
fig2
```
