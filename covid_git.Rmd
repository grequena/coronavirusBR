...
dfshow <- df_cont %>% 
  ...
  dplyr::select(1, 2, 7, 3, 8, 4, 9, 5, 6)

# names(dfshow)[c(3, 5, 7)] <- "%"
# names(dfshow)[8] <- "Taxa de<br>Recuperação" # with escape=F in DT::datatable
# names(dfshow)[9] <- "Taxa de<br>Mortalidade" 
...
%M $= \frac{Mortes}{Total(Mortes)}$,
Taxa de recuperação $= \frac{Recuperados}{Confirmados}$ e 
...
dfshow <- df %>% 
  ...
  dplyr::select(1, 2, 7, 4, 8, 3, 9, 5, 6)
...
dfshow <- df_pais %>% 
  ...
  dplyr::mutate(
    ...
    `%C` = 
      paste0(
        format(
          round(100*Confirmados/sum(df$confirmed), 1),
          decimal.mark = ",", big.mark = "."
        ),
        "%"
      ),
    `%R` = 
      paste0(
        format(
          round(100*Recuperados/sum(df$recovered), 1),
          decimal.mark = ",", big.mark = "."
        ),
        "%"
      ),      
    `%M` = 
      paste0(
        format(
          round(100*Mortes/sum(df$death), 1),
          decimal.mark = ",", big.mark = "."
        ),
        "%"
      ), 
...
sketch <- htmltools::withTags(table(
    DT::tableHeader(dfshow),
    DT::tableFooter(sapply(dfshow,
                           function(x)ifelse(is.numeric(x),sum(x),"Total")))))

names(dfshow)[1] <- c("País/Região")

DT::datatable(dfshow, rownames = F,
              options = list(
                dom = 't',
                columnDefs = list(
...
```{r}
plotly::plot_ly(data = dfshow[1:10, ], 
                x = ~ `País/Região`, 
                y = ~ Confirmados, 
                type = "bar", 
                name = "Confirmados",
...
### **<span style="color: #1478BE;">CONFIRMADOS</span>** na **América do Sul**
...
### **<span style="color:#0A780AE6">RECUPERADOS</span>** na **América do Sul**
...
### **<span style="color:#DC2828E6">MORTES</span>** na **América do Sul**
