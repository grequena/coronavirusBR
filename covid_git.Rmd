...
download.file("https://data.brasil.io/dataset/covid19/caso_full.csv.gz",
              temp)
R.utils::gzip(temp, ext = "gz")
d.munic <- read.csv("caso_full.csv", header=T, sep=",") %>%
...
d.estado <- d.munic %>%
  ...
  dplyr::arrange(date) %>% 
  dplyr::select(date, state, last_available_confirmed,
                last_available_deaths,
                estimated_population_2019) %>% 
  dplyr::rename(confirmed = last_available_confirmed,
...
```{r}
m <- 7
country <- c(as.character(df$Country.Region[1:m]), "Iran", "China")
...
set.seed(2009)
cores <- sample(c(RColorBrewer::brewer.pal(8, "Dark2"),
...
df_pais_500 <- df_pais %>%
  dplyr::group_by(Country.Region) %>%
  dplyr::filter(date >= min(lubridate::ymd(date[confirmed_cum>=day.d]))) %>%
  dplyr::mutate(dia=1:sum(date>=min(lubridate::ymd(date[confirmed_cum>=day.d]))))
...
for(i in 1:m) {
  ...
  fig2 <- plotly::add_trace(fig2,
                            ...
                            line = list(width = 1,
                                        color = cores[i])) %>% 
    plotly::add_text(x = max(aux.df$dia),
                     y = aux.df$confirmed_cum[
                       aux.df$dia == max(aux.df$dia)],
                     text = ifelse(country[i] == "United Kingdom",
                                          "UK", country[i]),
                     showlegend = F,
                     textposition = ifelse(
                       country[i]=="France", "middle left", 
                       ifelse(
                         country[i]%in%c("Germany", "Belgium"), "top right",
                         "outside"))
    )
}
aux.df <- df_pais_500 %>%
...
fig2 <- plotly::add_trace(fig2,
                          ...
                          line = list(width = 1,
                                      color = cores[i])) %>% 
  plotly::add_text(x = max(aux.df$dia),
                   y = aux.df$confirmed_cum[aux.df$dia == max(aux.df$dia)],
                   text = "Brazil",
                   showlegend = F)
                   # textposition = "right")
...
data.maxmin <- d.estado %>% 
  dplyr::group_by(state) %>% 
  dplyr::filter(date == max(date)) %>% 
  dplyr::ungroup() %>% 
  dplyr::select(date) 
data.minmax <- min(data.maxmin$date)

dfshow1 <- d.estado %>% 
  dplyr::filter(date <= data.minmax) %>% 
