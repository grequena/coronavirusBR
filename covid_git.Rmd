...
```{r}
df_cont$continent <- factor(df_cont$continent, levels = 
                              rev(c("Oceania", "Africa", "Americas", "Europe",
                                    "Asia")))
...
DT::datatable(dfshow, rownames = F,
              ...
              container = sketch) %>% 
  DT::formatStyle("Continente", target = "row",
                  backgroundColor = DT::styleEqual(
                    rev(dfshow$Continente),
                    viridis::viridis(5, option = "D", alpha = .4)))
```
...
Países mais afetados {data-orientation=rows}
================================================================================
...
names(dfshow) <- c("País/Região", "Confirmados", "Recuperados", "Mortes")
DT::datatable(dfshow, options = list(
  columnDefs = list(list(rownames = F, 
                         className = 'dt-center', 
                         targets = "_all")),
  lengthChange = FALSE))
